...
like($ans[1]->[0] && $ans[1]->[0]->address, qr/192.168.1.[234]/, 'correct A record came back');

ok(@ans = $g->reply_handler("foo.example.com", "IN", "ANY", "192.168.0.10"), "ANY request");
like((map { $_->address } grep { $_ && $_->type eq 'A' } @{ $ans[1] })[0], qr/192.168.1.[234]/, 'correct A record came back');
